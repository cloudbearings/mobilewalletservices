...
import com.mobilewallet.credits.dto.WalletHistoryBean;
...
public class WalletHistoryService {
    ...
    private final Log log = LogFactory.getLog(WalletHistoryService.class);

    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Object creditHistory(@QueryParam("userId") String eid, @QueryParam("b") String b) {
        ...
        if (userId != null) {
            ...
            if (user != null) {
                ...
                WalletHistoryBean crhb = null;

                int count = CreditsBO.creditCount(user.getUserId());
                ...
                if (count > 0) {
                    ...
                    List<WalletHistoryDTO> ctrList = CreditsBO.creditHistory(user.getUserId(), begin, end);

                    if (ctrList != null && !ctrList.isEmpty()) {
                        crhb = new WalletHistoryBean();
                        crhb.setCtrList(ctrList);
                        crhb.setCount(count);
                        crhb.setBegin(begin);
                        crhb.setEnd(end);
                        return crhb;
